...
import { Search, ShoppingCart, Printer, Trash2, MoreHorizontal } from "lucide-react";
...
// NOTE: Renaming component from PurchaseOrders to Purchases to align with context
// And using usePurchases hook instead of usePurchaseOrders
const PurchaseOrders = () => {
  ...
  const getAugmentedPurchases = () => {
      return purchases.map(p => {
          const totalHT = p.items.reduce((sum, item) => sum + item.total, 0);
          const totalTVA = p.items.reduce((sum, item) => {
  ...
  const filteredPurchases = getAugmentedPurchases().filter(purchase => 
  ...
  const getStatusClass = (status: string) => {
    switch (status) {
      ...
      case "pending":
        return "bg-pastel-yellow text-amber-800";
      case "canceled":
        return "bg-pastel-pink text-red-800";
      default:
  ...
  const handlePrintPurchase = (purchase: any) => {
    ...
    const supplier = allSuppliers.find(s => s.name === purchase.supplierName);

    if (!supplier) {
        toast({ title: "Erreur", description: `Fournisseur '${purchase.supplierName}' non trouvé.`, variant: "destructive" });
        return;
    }
    
    const htmlContent = generateModernPurchaseOrderHTML(purchase, supplier, formatCurrency, settings);
              ...
              {filteredPurchases.length > 0 ? (
                filteredPurchases.map((purchase) => (
                  <TableRow key={purchase.id}>
                    ...
                    <TableCell className="font-semibold text-blue-600">{formatCurrency(purchase.totalTTC)}</TableCell>
                    <TableCell>
                      <Badge className={getStatusClass(purchase.status)}>
                  ...
                  <TableCell colSpan={6} className="text-center py-8">
                    <div className="flex flex-col items-center gap-2">
                      ...
                      <p className="text-muted-foreground">
                        {searchTerm ? "Aucun bon de commande trouvé" : "Aucun bon de commande enregistré"}
                      </p>
                      {!searchTerm && (
                        <Link to="/purchases/add">
                          <Button className="mt-2">Créer un bon de commande</Button>
                        </Link>
                      )}
                    </div>
                  </TableCell>
