...
import { Search, Plus, Printer, Check, X, FileText, FilePlus2, RefreshCw } from "lucide-react";
import { useToast } from "@/hooks/use-toast";
import { useCompanySettings } from "@/contexts/CompanySettingsContext";
import { useQuotes } from "@/contexts/QuotesContext";

const generateModernQuoteHTML = (quote: any, client: any, formatCurrency: (amount: number) => string, settings: any) => {
  ...
  const tvaBreakdown = (quote.items || []).reduce((acc: any, item: any) => {
  ...
  const displayItems = (quote.items || []).map((item:any) => {
  ...
  return `
    ...
    <head>
      <meta charset="UTF-8">
      <title>Devis ${quote.reference || quote.id}</title>
      <style>
      ...
      <div class="invoice-container">
        <header class="invoice-header">
          ...
          <div class="invoice-section">
            <h2>DEVIS</h2>
            <div class="invoice-number">${quote.reference || quote.id}</div>
            <div class="invoice-date">Date: ${new Date(quote.date).toLocaleDateString('fr-FR')}</div>
            <div class="invoice-date">Valide jusqu'au: ${new Date(quote.validUntil).toLocaleDateString('fr-FR')}</div>
          </div>
        </header>
        <main class="content-wrapper">
          <div class="client-section">
            ...
            <div class="client-details">
                <p class="client-name"><strong>${client?.name || quote.clientName}</strong></p>
          ...
          <div class="totals-section">
            ...
            <div class="invoice-totals">
              <p><strong>Montant Total HT:</strong> <span>${formatCurrency(quote.totalHT || 0)}</span></p>
              <p><strong>FODEC 1%:</strong> <span>${formatCurrency(quote.fodec || 0)}</span></p>
              <p><strong>Total TVA:</strong> <span>${formatCurrency(quote.totalTVA || 0)}</span></p>
              <p><strong>Timbre Fiscal:</strong> <span>${formatCurrency(quote.timbreFiscal || 0)}</span></p>
              <p class="total-ttc"><strong>Total TTC:</strong> <span>${formatCurrency(quote.totalTTC || 0)}</span></p>
            </div>
          </div>
          <div class="footer-info">
             <p>Arrêté le présent devis à la somme de: <strong>${numberToWords(quote.totalTTC || 0)}</strong>.</p>
          </div>
...
const Quotes = () => {
  const { quotes, updateQuote, loading } = useQuotes();
  ...
  const filteredQuotes = quotes.filter(quote => {
    ...
    if (activeTab === "pending") return matchesSearch && quote.status === "pending";
  ...
  const getStatusBadge = (status: Quote["status"]) => {
    switch (status) {
      ...
      case "accepted":
        return <Badge className="bg-pastel-green text-green-800">Accepté</Badge>;
      case "rejected":
        return <Badge className="bg-pastel-pink text-red-800">Refusé</Badge>;
      case "expired":
        return <Badge variant="outline" className="text-gray-600">Expiré</Badge>;
      default:
        return null;
    }
  };
  ...
  const handlePrint = (quote: Quote) => {
    ...
    if (!client) {
        toast({ title: "Erreur", description: `Client avec ID ${quote.clientId} non trouvé.`, variant: "destructive" });
        return;
    }
    ...
    const htmlContent = generateModernQuoteHTML(quote, client, formatCurrency, settings);
  ...
  async function handleAcceptQuote(id: string) {
    try {
      ...
      toast({
        title: "Devis accepté",
        description: "Le statut du devis a été changé à Accepté",
        variant: "default",
      });
    } catch (error) {
  ...
  async function handleRejectQuote(id: string) {
    try {
      ...
      toast({
        title: "Devis refusé",
        description: "Le statut du devis a été changé à Refusé",
        variant: "destructive",
      });
    } catch (error) {
  ...
  const handleCreateInvoiceFromQuote = (quote: Quote) => {
    navigate('/sales/add', { state: { quoteData: quote } });
  };
        ...
        {filteredQuotes.length > 0 ? (
                ...
                <div className="p-6">
                  <div className="flex items-start justify-between mb-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <h3 className="text-lg font-semibold">Devis #{quote.reference}</h3>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    ...
                    <div>
                      <h4 className="text-sm font-medium mb-1">Produits ({quote.items.length}):</h4>
                      <ul className="text-sm list-disc ml-5">
                        ...
                        {quote.items.length > 2 && (
                          <li className="text-muted-foreground">
                            + {quote.items.length - 2} autres produits
                          </li>
                        )}
                      </ul>
                    </div>
                ...
                <div className="border-t p-4 bg-muted/20 flex items-center justify-between">
                  <div>
                    <div className="text-sm text-muted-foreground">
                      Total HT: {quote.totalHT.toFixed(3)} TND |
                      TVA: {quote.totalTVA.toFixed(3)} TND
                      {quote.fodec && quote.fodec > 0 && ` | FODEC: ${quote.fodec.toFixed(3)} TND`}
                      {quote.timbreFiscal && quote.timbreFiscal > 0 && ` | Timbre: ${quote.timbreFiscal.toFixed(3)} TND`}
                    </div>
                    <div className="text-lg font-semibold">
                      Total TTC: {quote.totalTTC.toFixed(3)} TND
                    </div>
                  </div>
                  <div className="flex items-center space-x-2">
                    <Button 
          ...
          <div className="text-center py-10">
            ...
            <p className="text-muted-foreground mb-4">
              Vous n'avez pas encore créé de devis ou votre recherche ne correspond à aucun résultat.
            </p>
            <Link to="/quotes/add">
              <Button>
